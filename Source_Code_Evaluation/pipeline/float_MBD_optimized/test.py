...
def read_and_process_file(line):
    ...
    mdr = record.get("parameters").get("MAX_PLAUSIBLE_DIST")
    threshold = record.get("parameters").get("threshold")
    iteration = record.get("iteration")
    weightCatchCheck = record.get("parameters", {}).get("weightCatchChecks", [])
    ...
    command = (
        ...
        f"-n {weight_args[6]} -o {weight_args[7]} -p {threshold} -q {iteration} -r {train} -s {attackType} -t {weight_args[8]} -u {mdr}"
    )

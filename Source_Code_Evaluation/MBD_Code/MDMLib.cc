...
double MDMLib::calculateHeadingAnglePtr(veins::Coord * heading) {
    ...
    double angle = atan2(det, dot) * 180 / M_PI; // atan2(y, x) or atan2(sin, cos);
...
double MDMLib::calculateDeltaTime(BasicSafetyMessage * bsm1,
    ...
    return bsm1->getArrivalTime() - bsm2->getArrivalTime();
}
...
double MDMLib::CircleCircleFactor(double d, double r1, double r2,
    ...
    if (r1 <= 0 && r2 <= 0) {
        ...
        if (range / 2 >= d1) {
            ...
            double factor = (area2) / (M_PI * r2 * r2);
            return factor;
        } else {
            return 0;
        }
        ...
        if (range / 2 >= d2) {
            ...
            double factor = (area1) / (M_PI * r1 * r1);
            return factor;
        } else {
            return 0;
        }
        ...
        double area1 = calculateCircleCircleIntersection(r1, range / 2, d1);
        double area2 = calculateCircleCircleIntersection(r2, range / 2, d2);
        double factor = (area1 + area2) / (M_PI * r1 * r1 + M_PI * r2 * r2);
        return factor;
    }
}
        ...
        if (range < d - r1 - r2) {
            ...
            if (r1 <= 0 && r2 <= 0) {
                ...
                if (range / 2 >= d1) {
                    ...
                    double factor = (area2 - area22) / (M_PI * r2 * r2);
                    return factor;
                } else {
                    return 0;
                }
                ...
                if (range / 2 >= d2) {
                    ...
                    double factor = (area1 - area12) / (M_PI * r1 * r1);
...
inline double noCircles = 0;

void MDMLib::countCircles(double rc, double rl, double rs) {
    double no = (int) ((2 * M_PI * rc) / (2 * rs));
    double x0 = rc * cos(0 * 2 * M_PI / no);
    double y0 = rc * sin(0 * 2 * M_PI / no);
    double x1 = rc * cos(1 * 2 * M_PI / no);
    double y1 = rc * sin(1 * 2 * M_PI / no);
    double dist = pow((pow((x0 - x1), 2) + pow((y0 - y1), 2)), 0.5);
    if (dist < 2 * rs)
        no = no - 1;
    for (int i = 0; i < no; i++) {
//        double x = rc * cos(i * 2 * M_PI / no);
//        double y = rc * sin(i * 2 * M_PI / no);
...
//    double areaFactor2 = areaIntersection / (PI * r2 * r2);
    ...
    double areaFactor = areaIntersection
            / ((M_PI * r1 * r1) + (M_PI * r2 * r2) - areaIntersection);
...
//    double factor = (factor1 * areaFactor1 + factor2 * areaFactor2)/2;
    ...
    double maxFactor = 1
            - ((interDistance * interDistance * M_PI) / (r1 * r1 * M_PI));
...
double MDMLib::EllipseEllipseIntersectionFactor(veins::Coord pos1, veins::Coord posConf1,
    ...
    double areaFactor = intArea
            / ((M_PI * (dx1 / 2) * (dy1 / 2)) + (M_PI * (dx2 / 2) * (dy2 / 2))
                    - intArea);
    ...
    double c1 = centersAngle * M_PI / 180;
    double h1 = heading1 * M_PI / 180;
    double h2 = heading2 * M_PI / 180;

    double diffAngle1 = atan2(sin(h1 - c1), cos(h1 - c1));
    double diffAngle2 = atan2(sin(h2 - c1), cos(h2 - c1));

    if (diffAngle1 > M_PI / 2) {
        diffAngle1 = diffAngle1 - M_PI;
    }
    if (diffAngle1 < -M_PI / 2) {
        diffAngle1 = diffAngle1 + M_PI;
    }
    if (diffAngle2 > M_PI / 2) {
        diffAngle2 = diffAngle2 - M_PI;
    }
    if (diffAngle2 < -M_PI / 2) {
        diffAngle2 = diffAngle2 + M_PI;
    }
